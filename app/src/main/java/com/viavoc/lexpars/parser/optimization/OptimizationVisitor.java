...
public abstract class OptimizationVisitor<T> implements ResultVisitor<Node, T> {
    ...
    @Override
    public Node visit(TypedAssignmentExpression s, T t){
        return this.visit(new AssignmentExpression(s.operation, s.target, s.expression), t);
    }
    ...
    @Override
    public Node visit(ClassStatement s, T t) {
        boolean changed = false;
        final BlockStatement result = new BlockStatement();
        for (Statement statement : s.fields) {
            final Node node = statement.accept(this, t);
            if (node != statement) {
                changed = true;
            }
        ...
        for (Statement statement : s.construstos) {
            final Node node = statement.accept(this, t);
            if (node != statement) {
                changed = true;
            }
        ...
        for (Statement statement : s.methods) {
            final Node node = statement.accept(this, t);
            if (node != statement) {
                changed = true;
            }
    ...
    public Node visit(TryCatchStatement s, T t) {
        boolean changed = false;
        final BlockStatement result = new BlockStatement();
        for (Statement statement : ((BlockStatement) s.tryBody).statements) {
            final Node node = statement.accept(this, t);
            if (node != statement) {
                changed = true;
            }
    ...
    @Override
    public Node visit(ForEachStatement s, T t) {
        final Node container = s.container.accept(this, t);
        final Node body = s.body.accept(this, t);
        if (container != s.container || body != s.body) {
            return new ForEachStatement(s.variable, (Expression) container, consumeStatement(body));
        }
        return s;
    }
    ...
    public Node visit(MatchExpression s, T t) {
        ...
        for (MatchExpression.Pattern pattern : s.patterns) {
            if (pattern instanceof MatchExpression.VariablePattern) {
                final String variable = ((MatchExpression.VariablePattern) pattern).variable.toString();
    ...
    public Node visit(NewExpression s, T t) {
        //final Node expression = s.name.accept(this, t);
        return s;
    }
    ...
    public Node visit(ValueExpression s, T t) {
        if ( (s.value.type() == Type.FUNCTION) && (s.value.raw() instanceof UserDefinedFunction) ) {
    ...
    protected boolean visit(final Arguments in, final Arguments out, T t) {
        ...
        for (Argument argument : in) {
            ...
            if (valueExpr == null) {
                ...
                out.addOptional(new StringValue(argument.getName()), (Expression) expr);
            }
        }
        return changed;
    }
